...
function getSubtitles(url){if(url!=="")subsGetter.sendMessage(url)
else subsGetter.sendMessage(streamUrl)
}function setSubtitles(subtitles){currentVideoSub=subtitles
}WorkerScript{id:subsGetter
source:"getsubtitles.js"
onMessage:{setSubtitles(messageObject)
}}function checkSubtitles(){subsChecker.sendMessage({"position":videoPoster.position,"subtitles":currentVideoSub})
}WorkerScript{id:subsChecker
source:"checksubtitles.js"
onMessage:{if(!isSolid)subtitlesText.text=messageObject
else subtitlesTextArea.text=messageObject
}}function contrastingColor(color){var rgb=getRGB(color)
console.debug("[SubtitlesItem.qml] getrgb:"+rgb)
if(!rgb)return null
return(0.2126*rgb[0]+0.7152*rgb[1]+0.0722*rgb[2])>180?"black":"white"
}function getRGB(b){var a
...
onTextChanged:{}style:Text.Outline
styleColor:contrastingColor(color)
}TextEdit{id:subtitlesTextArea
z:100
anchors.fill:parent
wrapMode:rootItem.wrapMode
horizontalAlignment:rootItem.horizontalAlignment
