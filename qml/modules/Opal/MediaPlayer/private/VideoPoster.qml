...
readonly property bool _loaded:active&&videoItem.player&&videoItem.player.status>=MediaPlayer.Loaded&&videoItem.player.status<MediaPlayer.EndOfMedia
implicitWidth:poster.implicitWidth
implicitHeight:poster.implicitHeight
function ffwd(seconds){seekNotice.secs=seconds
seekNotice.isRewind=false
seekNotice.show()
videoItem.player.seek((positionSlider.value*1000)+(seconds*1000))
}function rew(seconds){seekNotice.secs=seconds
seekNotice.isRewind=true
seekNotice.show()
videoItem.player.seek((positionSlider.value*1000)-(seconds*1000))
}NoticeLabel{id:seekNotice
property int secs:10
property bool isRewind:false
opacity:0.0
visible:opacity>0.0
Behavior on opacity{FadeAnimator{}}anchors{verticalCenter:parent.verticalCenter
...
}Connections{target:videoItem._loaded?videoItem.player:null
...
}onActiveChanged:{if(!active){positionSlider.value=0
}}Thumbnail{id:poster
...
function forward(seconds){ffwd(seconds)
}function rewind(seconds){rew(seconds)
}property int _clickCount:0
Timer{id:multiClickTimer
running:false
interval:1500
onTriggered:stop()
triggeredOnStart:false
}CircleButton{anchors{verticalCenter:playPauseButton.verticalCenter
right:playPauseButton.left
rightMargin:Theme.paddingLarge
}icon.source:"images/icon-m-rewind.png"
grow:0.8*Theme.paddingLarge
onClicked:{if(!seekNotice.isRewind){controls._clickCount=0
multiClickTimer.stop()
}if(multiClickTimer.running){controls._clickCount+=1
controls.rewind(5*controls._clickCount)
}else{controls._clickCount=1
multiClickTimer.start()
controls.rewind(5)
}}}CircleButton{id:playPauseButton
anchors.centerIn:parent
icon.source:_isPlaying?"images/icon-m-pause.png":"images/icon-m-play.png"
...
multiClickTimer.start()
controls.forward(10)
}}}SilicaControl{anchors{left:parent.left
right:parent.right
...
onClicked:{toggleAspectRatio()
}}Label{id:maxTime
visible:videoItem._loaded||!!text
anchors{right:aspectBtn.left
rightMargin:aspectBtn.visible?Theme.paddingMedium:(2*Theme.paddingLarge)
bottom:parent.bottom
bottomMargin:Theme.paddingLarge
}text:{if(positionSlider.maximumValue>3599){return Format.formatDuration(positionSlider.maximumValue,Formatter.DurationLong)
}else{return Format.formatDuration(positionSlider.maximumValue,Formatter.DurationShort)
}}}IconButton{id:repeatBtn
icon.source:repeat?"image://theme/icon-m-repeat":"image://theme/icon-m-repeat-single"
...
height:width
visible:jupii.found
onClicked:{jupii.addUrlOnceAndPlay(streamUrl.toString(),streamTitle,"",(onlyMusic.visible?1:2),"OpalMediaPlayer",Qt.resolvedUrl("images/icon-m-cast.png").toString().replace("file://",""))
}}Slider{id:positionSlider
anchors{left:castBtn.right
right:maxTime.left
bottom:parent.bottom
...
minimumValue:0
valueText:{if(value>3599){return Format.formatDuration(value,Formatter.DurationLong)
}else{return Format.formatDuration(value,Formatter.DurationShort)
}}onReleased:{if(videoItem.active){videoItem.player.source=videoItem.source
