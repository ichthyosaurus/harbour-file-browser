...
// TODO and FIXME:
// Calling static methods from QML is not possible, even though
// it looks like it. You always get "TypeError: Property 'myMethod' of object [object Object] is not a function"
// It is only possible through a singleton type, and a singleton type
// must be a QObject, which is trouble.
...
#define CREATE_ENUM(NAME, VALUES...) \
    ...
    public: \
        ...
        Q_INVOKABLE static bool isValid(int value) { \
            return !QMetaEnum::fromType<Enum>().valueToKeys(value).isEmpty(); \
        } \
        \
        Q_INVOKABLE static QString string(int value) { \
            return QString::fromLatin1(QMetaEnum::fromType<Enum>().valueToKeys(value)); \
        } \
        \
        explicit NAME() { throw std::logic_error("This class is only a container for an enum."); }; \
    }; \
...
#define REGISTER_ENUM_CONTAINER(NAME) \
    ...
    qRegisterMetaType<QList<NAME>>("QList<" #NAME ">"); \
